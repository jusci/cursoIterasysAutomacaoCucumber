...
public class ComprarProdutoSteps {
	...
	@Entao("o produto aparece na confirmacao com nome {string} e preco {string} tamanho {string} cor {string} e quantidade {int}")
	public void o_produto_aparece_na_confirmacao_com_nome_e_preco_tamanho_cor_e_quantidade(String nomeProduto,
	...
	@After ( order = 1) // numeros mais altos são executado primeiro
	//esse método são para o captura de telas via cucumber
	...
	public void capturarTela(Scenario scenario) {
		...
		String scenarioId = scenario.getId().substring(scenario.getId().lastIndexOf(".feature:") + 9 );// pega o que vem depois da posição 9 ( recupera o ID)
	...
	@After ( order = 0) // numero menores são executado por ultimo
